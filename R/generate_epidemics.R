...
#' @title * SI model
#' @title * all individuals are infected during the epidemic (prevalence = 1)a
#'
#' @import MASS
#'
...
generate_epidemics <-
  function(offspring_data_replicates,
           group_size,
           seed = 1754) {
    set.seed(seed)
    ...
    for (replic in 1:max(offspring_data_replicates$replicate)) {
      offspring_data <-
        offspring_data_replicates[offspring_data_replicates$replicate ==
                                    replic,]
      ...
      for (i in 1:number_of_groups) {
        ...
        while (sum(data_group_i$is_infected) / group_size < 1) {
          ...
          next_infection_time <-
            next_infection_time + rexp(1, rate = sum(lambda[[i]]))
          ID_next_inf <-
            sample(data_group_i$ID,
                   size = 1,
                   prob = lambda[[i]] / sum(lambda[[i]]))
          data_group_i[data_group_i$ID == ID_next_inf, "is_infected"] <-
            1
          data_group_i[data_group_i$ID == ID_next_inf, "infection_time"] <-
            next_infection_time
        }
    ...
    return(offspring_epidemic_data_replicates[order(
      offspring_epidemic_data_replicates$replicate,
      offspring_epidemic_data_replicates$group,
      offspring_epidemic_data_replicates$infection_time
